...
def _slope_derived_structure(part, slope_difference_threshold=0.05):
    """Derive clonal structure from VAF slopes using union-find.

    Mutations whose VAF/year slopes differ by <= threshold are grouped
    into the same clone. This is the anchor for model selection — the HMM
    evaluates only this structure rather than all valid partitions.
    """
    AO = part.layers["AO"].astype(float)
    DP = part.layers["DP"].astype(float)
    vaf = AO / np.maximum(DP, 1.0)   # (n_mutations, n_timepoints)
...
def assign_clonal_structure_from_slopes(part, slope_difference_threshold=0.05):
    ...
    """
    cs = _slope_derived_structure(part, slope_difference_threshold)
...
def compute_deterministic_size_mixed(cs, AO, DP, n_mutations, N_w=1e5):
    """Compute fixed-h clone sizes used to initialise N_w_cond for the HMM.

    Uses an adaptive h_min floor with a generous gap (0.15) above the peak
    observed VAF so that the inversion denominator (v - (1+h)/2) is always
    well-conditioned, even when the observed VAF approaches 0.5.
    """
    ...
    deterministic_clone_size = -N_w * leading_vaf_sum / (
        leading_vaf_sum - (1.0 + h_min) / 2.0)
...
def compute_model_likelihood_2d(output, cs, s_range, h_range):
    s_prior = 1.0 / (s_range.max() - s_range.min())
    h_prior = 1.0 / (h_range.max() - h_range.min())

    clonal_prob = np.ones(len(cs))
    for clone_index in range(len(cs)):
        grid = np.array(output[:, :, clone_index], copy=False)
        grid = np.nan_to_num(grid, nan=0.0, posinf=0.0, neginf=0.0)
        grid = np.maximum(grid, 0.0)

        int_h = np.trapz(x=h_range, y=grid, axis=1)
        int_sh = np.trapz(x=s_range, y=int_h)
        clonal_prob[clone_index] = s_prior * h_prior * max(int_sh, 0.0)

    return float(np.prod(clonal_prob))
...
def compute_clonal_models_prob_vec_mixed(
    ...
    slope_difference_threshold=0.05,
):
    ...
    this function:
      1. Derives the clonal structure directly from VAF slopes (union-find).
    ...
    This avoids the normalisation pathology that occurs when most structures
    have zero HMM probability but one has a tiny non-zero value.
    """
    ...
    model_prob = compute_model_likelihood_2d(output, cs, s_vec, h_vec)
    part.uns["model_dict"]["model_0"] = (cs, model_prob)

    # Normalisation — handle zero/non-finite total gracefully
    total_prob = model_prob
    if total_prob > LIKELIHOOD_FLOOR and np.isfinite(total_prob):
        norm_prob = 1.0
    else:
        part.uns["warning"] = "HMM probability zero — slope structure used directly"
        norm_prob = 1.0   # only one model, so normalised prob is always 1

    part.uns["model_dict"]["model_0"] = (cs, model_prob, norm_prob)
...
def compute_invalid_combinations(part, slope_difference_threshold=0.05):
    ...
    Replaces the old Pearson-correlation-based filter, which was degenerate
    with only 2 timepoints (correlation is always ±1 for any monotone series).
    """
    AO = part.layers["AO"].astype(float)
    DP = part.layers["DP"].astype(float)
...
def _project_clone_vaf(
    ...
    Parametrisation:
        x_hom = h * x_het,  x_tot = x_het * (1 + h)

    Expected VAF:
        VAF = x_tot * (1 + 2h) / (2 * (1 + h) * (N_w + x_tot))
    """
    ...
    return np.clip(projected_vaf, EPS, 1.0 - EPS)
...
def infer_sh_jointly_from_dynamics(
    ...
    for clone_index, clone_mutations in enumerate(cs):
        ...
        s_ci = (
            s_range[min(np.searchsorted(s_cumsum, 0.05), len(s_range) - 1)],
            s_range[min(np.searchsorted(s_cumsum, 0.95), len(s_range) - 1)],
        )
        h_ci = (
            h_range[min(np.searchsorted(h_cumsum, 0.05), len(h_range) - 1)],
            h_range[min(np.searchsorted(h_cumsum, 0.95), len(h_range) - 1)],
        )

        denom = (1.0 + 2.0 * h_map) - 2.0 * observed_vaf[0] * (1.0 + h_map)
        denom = max(denom, EPS)
        initial_mutant_cells = max(
            2.0 * observed_vaf[0] * (1.0 + h_map) * N_w / denom,
            100.0,
        )
...
def refine_optimal_model_posterior_vec_mixed(
    ...
    posterior_2d = np.stack(
        [np.outer(result["s_posterior"], result["h_posterior"]) for result in joint_results],
        axis=2,
    )
